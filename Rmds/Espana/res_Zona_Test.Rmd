---
title: "Resultados"
author: "Carlos Meijide Garcia y Marcos Matabuena""
...
reais_raw=read.table("../../src/Results/Espana/Zona_Test/SimulaZona_Test_XM_Real.txt")
...
# Se lee la información de población de todas las autonomías
autonomias=read.csv("../../Data/Espana/Espana_Poblacion.csv",header=TRUE,sep=',')

# Se lee el listado de fallecidos de todas las autonomías
...
# Se cuenta cuantos ceros hay al inicio del listado de muertes
nceros=0
j=0
while(reaisi[j+3]==0) {
  nceros=nceros+1
...
# Se hace el cálculo de la fecha de inicio usando el número de columnas del archivo de muertes
d=ncol(mortos_raw)
r= 18-nceros #numero de dias con antelacion cos que empeza o simulador

# Aquí se debería leer la fecha de inicio desde el archivo "Espana_Fallecidos.csv"
...
# Se leen los parámetros de la R0 correspondientes a cada localidad
parametros= read.table("../../src/Results/Espana/Zona_Test/SimulaZona_Test_Param.txt")
parametros= parametros[19:22]

# Se construyen los valores de la función a partir de los parámetros leidos
fn= function(t, parametros){
  result=numeric(length(t))
  for(i in 1:length(t)){
    result[i]= parametros[1]/(1+parametros[2]^(-(t[i]-parametros[3])))+parametros[4]
  }
  result=unlist(result)
  return(result)
}

# Se pinta la función R0.
mortos_rawd= ncol(mortos_raw)
r0t= fn(1:(mortos_rawd-1), parametros )  
dias= colnames(mortos_raw)[-c(1)]
plot(r0t~as.Date(dias), type="l", wd=3 , col=rgb(0.1,0.7,0.1,0.8) , ylab="R0(t)" , xlab="date" , bty="l" , pch=20 , cex=4)
